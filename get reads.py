...
in the V_genes_dict by one, then returns that dictionary'''
    ...
    for line in f:
        ...
        if a[2] == 'productive':                              #if it is a productive rearrangement
            ...
            List.append(reads)   #gets the IgV region, before the allele nomenclature '*' and after the species nomenclature 'Musmus'
...
for item in names:
    ...
    new_list = reads(file1)
    CDR3_dict[str(item[3:] + '-' + item[:2])] = new_list     #rearranges file names to a more easily sortable name
...
##export_location = '/Users/haochuhuang/Documents/Jason Perera/3H9 BCR HTS Sequencing/Prism and Excel/reads.txt'
##file2 = open(export_location, 'w')
##
##  
##for i in range(20):
##    file2.write(CDR3_dict[names2[i]])
##    file2.write('\n')
